...
```
The test turns out to be correct and gives no error, since it is simply verifying that each component of the resulting vector was computed correctly, but it is manifest that the expected result is not **exactly** equal to the code result.
